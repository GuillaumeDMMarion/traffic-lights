...
class TestLogger(Callback):
    ...
    def on_train_begin(self, logs):
        ...
        # Debug csv.
        try:
          with open("debug_test.csv") as file:
            file.read()
            file.close()
        except FileNotFoundError:
          with open("debug_test.csv", "a") as file:
            file.write("reward_total")
            file.close()

    def on_episode_end(self, episode, logs):
        """ Print logs at end of each episode """
        template = 'Episode {episode}: reward: {episode_reward:.3f}, steps: {nb_steps}'
        variables = {
            'episode': episode + 1,
...
class TrainEpisodeLogger(Callback):
    ...
    def on_step_end(self, step, logs):
        ...
        self.delays[episode].append(logs['info']['added_delay'])
        self.step += 1
